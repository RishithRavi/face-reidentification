...
from live_scaled import FaceDatabase, process_frame, setup_logging

def parse_args():
    parser = argparse.ArgumentParser(description="Test Face Recognition Scaling")
    parser.add_argument(
        "--det-weight",
        type=str,
...
def generate_synthetic_faces(real_faces, num_synthetic=1000) -> List[Tuple[np.ndarray, str]]:
...
def build_face_database(real_faces, synthetic_faces, similarity_threshold):
    """Build a face database with both real and synthetic faces."""
    database = FaceDatabase(similarity_threshold=similarity_threshold)
    
    # Add real faces first
    for embedding, name in real_faces:
        database.add_face(embedding, name)
    
    # Add synthetic faces
    for embedding, name in synthetic_faces:
        database.add_face(embedding, name)
    
    # Finalize database
    database.finalize()
    
    logging.info(f"Face database built with {len(database)} faces")
    return database
...
def main():
    ...
    # Build face database
    face_db = build_face_database(real_faces, synthetic_faces, params.similarity_thresh)
    ...
    window_name = f"Scaling Test: {len(face_db)} faces in database"
    ...
    try:
        while True:
            ...
            cv2.putText(
                processed_frame,
                f"FPS: {fps_display:.1f} | Faces: {num_faces} | DB Size: {len(face_db)} | Process: {process_time*1000:.1f}ms",
                (10, 30),
